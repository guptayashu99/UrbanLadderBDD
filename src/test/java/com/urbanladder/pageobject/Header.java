...
public class Header {
	...
	public static boolean getLoginPage(WebDriver driver) {
		flag = null;
		try {
			ReusableMethods.click(HeaderUI.trackorder, driver);
			ReusableMethods.click(LoginPageUI.loginclick, driver);
			flag = true;
			Logging.log("info", "Clicked on Login Page Button");
		}
		catch(Exception e) {
			flag = false;
			Logging.log("error", "Could not click on Login Page Button");
		}
		return flag;
	}
	
	public static boolean getCart(WebDriver driver) {
		flag = null;
		try {
			ReusableMethods.click(HeaderUI.cart, driver);
			flag = true;
			Logging.log("info", "Cart Button Clicked");
		}
		catch(Exception e) {
			flag = false;
			Logging.log("error", "Could not click on cart");
		}
		return flag;
	}
}
